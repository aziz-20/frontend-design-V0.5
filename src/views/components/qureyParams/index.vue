  ...
  <el-form :inline="true" :model="queryParams" ref="queryForm" size="small" class="demo-form-inline"
    ...
    <div class="queryParams-container">
      <el-form-item v-for="(field, index) in fields" :key="index">
        <label class="queryParams-label">{{ field.label }}
          <div class="queryParams-field">
            <template v-if="shouldShowField(field)">
              ...
              <template v-if="field.inputtype === 'departments'">
                <el-tree-select v-model="queryParams[field.name]" :data="field.deptemail? deptemail:(field.deptparent ? deptparent: department)" :render-after-expand="true"
              ...
              <template v-if="field.inputtype === 'userField'">
                <el-select-v2 v-model="queryParams[field.name]" :placeholder="field.placeholder"
                  :options="field.email ? email:username " collapse-tags collapse-tags-tooltip
                  :max-collapse-tags="3" filterable />
              </template>
